...
function counts() {
  return {
    ...
    freelance: state.jobs.filter(job => job.status === "new" && job.opportunityType === "freelance" && Number(job.score||0) >= 30).length,
...
function renderFreelance() {
  const freelanceJobs = state.jobs.filter(job => job.status === "new" && job.opportunityType === "freelance" && Number(job.score||0) >= 30);
  const hiddenLow = state.jobs.filter(job => job.status === "new" && job.opportunityType === "freelance" && Number(job.score||0) < 30).length;
...
async function addSource() {
  ...
  const organizationOptional = ["remoteok", "weworkremotely"].includes(provider);
  if ((!organization && !organizationOptional) || !label) return toast("Board identifier and company label are required.");
  try {
